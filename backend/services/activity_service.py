...
Simpler than fan-out on write, sufficient for InFinea's scale,
...
async def create_activity(
    ...
    # If private, don't even store it — no one will see it
...
async def get_feed(
    ...
    # Include the user's own activities in their feed
    ...
    # Enrich with user info (display name + avatar) — batch query
    if activities:
        enriched_user_ids = list({a["user_id"] for a in activities})
        users = await db.users.find(
            {"user_id": {"$in": enriched_user_ids}},
            {"_id": 0, "user_id": 1, "name": 1, "display_name": 1, "avatar_url": 1},
        ).to_list(len(enriched_user_ids))

        user_map = {u["user_id"]: u for u in users}

        for activity in activities:
            user_info = user_map.get(activity["user_id"], {})
            activity["user_name"] = user_info.get(
                "display_name", user_info.get("name", "Utilisateur")
            )
            activity["user_avatar"] = user_info.get("avatar_url")

        # Check if the requesting user has reacted to any of these
